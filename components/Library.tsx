...
interface LibraryProps {
  ...
  onOpenProfile: () => void;
  onAddNovel: (novel: Novel, categoryId: string) => void;
  ...
  return (
    <div className="min-h-screen bg-[#0f0f12] text-white pb-32 font-sans">
      ...
      <header className="sticky top-0 z-30 bg-[#0f0f12]/90 backdrop-blur-xl border-b border-white/5 supports-[backdrop-filter]:bg-[#0f0f12]/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center text-indigo-400 shadow-[0_0_20px_rgba(99,102,241,0.3)] ring-1 ring-indigo-500/30">
               <BookOpen size={20} />
            </div>
            <span className="font-display font-bold text-lg sm:text-xl tracking-tight hidden sm:block">Lumina</span>
          </div>
            ...
            >
                {categories.map(cat => (
                    <button
                        ...
                        className={`relative whitespace-nowrap px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 ${
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-12">
        ...
        {recentNovel && activeCategoryId === 'reading' && (
        ...
        <section>
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-xs text-gray-600 bg-white/5 px-2 py-1 rounded">{filteredNovels.length} Items</div>
            </div>
           
           <div key={activeCategoryId} className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-6 gap-y-10">
              {filteredNovels.map((novel, idx) => (
                ...
                >
                  <div className="relative aspect-[2/3] rounded-xl transition-all duration-300 group-hover:-translate-y-2 preserve-3d">
                     ...
                     <div className="relative w-full h-full rounded-xl overflow-hidden border border-white/10 shadow-lg group-hover:shadow-2xl">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-500">{novel.totalChapters} chs</span>
                    </div>
                  </div>
                </div>
              ))}
              
              {/* Import Card (Visible if empty or just always at end? Let's remove if list populated to keep clean, but show if needed) */}
                                      ...
                                      {categories.map(cat => (
                                          <button
                                            ...
                                            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                                                selectedQuickAddCategory === cat.id 
