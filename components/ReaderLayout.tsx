...
export const ReaderLayout: React.FC<ReaderLayoutProps> = ({
  ...
  const loadTOC = async () => {
    if (localMode) {
      const items = await localReader.getTOC();
      setToc(items);
    } else {
  ...
  const loadMoreChapters = async (startNum: number, clearExisting = false) => {
    ...
    try {
      ...
      if (localMode) {
        // For local mode, we need to get the TOC first if not loaded, but usually it should be.
        // We assume getChapterByIndex handles 1-based index
        // If TOC is empty, we might need to wait or handle it. 
        // For now, assume TOC is loaded or we can get it.
        // Actually, localReader stores the book instance, so we can just ask for chapter by index.
        // But we need the TOC to know the href if we use getChapter(href).
        // Let's use getChapterByIndex which we added to LocalReaderService.
        const currentToc = toc.length > 0 ? toc : await localReader.getTOC();
        if (toc.length === 0) setToc(currentToc);

        if (startNum > currentToc.length) {
          setLoading(false);
          return; // End of book
        }
        newChapter = await localReader.getChapterByIndex(startNum, currentToc);
      } else {
        newChapter = await fetchChapter(startNum, novelId);
      }
      ...
      <div className="pt-16 pb-32">
        {chapters.map((chapter) => (
          <ChapterNode
            ...
            chapter={chapter}
            settings={settings}
            onOpenComments={localMode ? undefined : (id) => setActiveCommentsChapterId(id)}
